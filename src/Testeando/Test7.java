...
public class Test7 {
    public static void main(String[] args) {
        ...
        String meterNumber;
        do {
            System.out.println("Ingrese el número de suministro (medidor) de 9 dígitos:");
            meterNumber = scanner.next();
            if (meterNumber.length() != 9) {
                System.out.println("Número de suministro incorrecto. Ingrese un número de suministro de 9 dígitos.");
            }
        } while (meterNumber.length() != 9);

        // Inicializar las pilas para almacenar datos
        Stack<String> periodStack = new Stack<>();
        Stack<Double> kWhStack = new Stack<>();
        Stack<Double> amountStack = new Stack<>();
        Stack<Double> debtStack = new Stack<>();
        Stack<Boolean> disconnectStack = new Stack<>();
        ...
        while (!meterRemoved) {
            ...
            // Verificar si el servicio está desconectado y el consumo es mayor a cero
            if (isDisconnected && kWhConsumed > 0) {
                System.out.println("ALERTA: Consumo detectado a pesar de que el servicio está cortado. Verifique el medidor.");
            }
            ...
            // Almacenar datos en las pilas
            periodStack.push(period);
            kWhStack.push(kWhConsumed);
            amountStack.push(amountToPay);
            debtStack.push(totalDebt);
            disconnectStack.push(isDisconnected);
        ...
        // Mostrar los datos almacenados en las pilas para análisis
        System.out.println("\n--- Resumen de Consumos y Pagos ---");
        while (!periodStack.isEmpty()) {
            String period = periodStack.pop();
            double kWh = kWhStack.pop();
            double amount = amountStack.pop();
            double debt = debtStack.pop();
            boolean disconnected = disconnectStack.pop();

            System.out.printf("Periodo: %s, Consumo: %.2f kWh, Monto: %.2f soles, Deuda: %.2f soles, Desconectado: %s%n",
                    period, kWh, amount, debt, disconnected ? "Sí" : "No");
        }
